...
class CaseModelFigure:
    ...
    def show(self):
        self.figure = cf.Figure(
            cf.subplots(self.figure_list, base_layout=self.figure_list[1].to_dict()['layout'], shape=(len(self.figure_list), 1)))
        self.figure.show()
    ...
    def get_figure(self, evidence, truth_value):
        ...
        figure.add_trace(
            go.Scatter(x=[1.2], y=[rough_height_indication_for_plot/2],
        ...
        for case in self.casemodel.cases:
            ...
            figure.add_trace(
                go.Scatter(x=[(width_of_case / 2) + prelude_width], y=[height_of_case],
                           text=[events_in_case], mode="text"))
            figure.add_trace(
                go.Scatter(x=[(width_of_case / 2) + prelude_width], y=[(height_of_case + rough_height_indication_for_plot/2)+ prelude_height],
